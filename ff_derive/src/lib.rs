...
impl ReprEndianness {
    ...
    fn decode_repr(&self, name: &syn::Ident, limbs: usize) -> proc_macro2::TokenStream {
...
fn fetch_attr(name: &str, attrs: &[syn::Attribute]) -> Option<String> {
    for attr in attrs {
        if attr.path().is_ident(name) {
            ...
            panic!("attribute {name} should be a name = \"string\" pair");
        }
    }
...
fn prime_field_impl(
    ...
    let from_repr_impl = endianness.decode_repr(name, limbs);
